...
첫 번째 검사는 그랜드 크로스 분배, 회차 간 금지 조합, 회차·역할군별 가속도 빠른 1명·느린 1명, 마안과 가속도 시간 연동, 전체 빠른 4명·느린 4명 배정, 3회차 디버프 인원을 무작위 상황 10,000개로 확인합니다. 두 번째 검사는 브라우저 스크립트 로딩 순서와 초기화 과정에 누락된 참조가 없는지 확인합니다.
